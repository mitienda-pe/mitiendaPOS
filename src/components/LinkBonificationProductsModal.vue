    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
          ...
          <div class="sm:flex sm:items-start">
            <div class="w-full">
              <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg leading-6 font-medium text-gray-900">Vincular Productos de Bonificación</h3>
                <button
                  @click="handleClose"
              ...
              <div class="space-y-4">
                ...
                <div v-else-if="!searchQuery" class="text-center py-8 text-gray-500">
                  ...
                  <p>Busca productos para bonificar</p>
                </div>

                <!-- Selected count -->
                <div v-if="selectedProductIds.length > 0" class="text-sm text-gray-700 bg-green-50 p-3 rounded-lg border border-green-200">
                  <svg class="inline h-5 w-5 text-green-600 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" />
                  </svg>
                  {{ selectedProductIds.length }} producto(s) seleccionado(s) para bonificar
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
          ...
          >
            {{ linking ? 'Vinculando...' : 'Vincular Bonificaciones' }}
          </button>
          <button
            @click="handleClose"
            :disabled="linking"
...
async function searchProducts() {
  ...
  try {
    ...
    const response = await productsApi.getProducts({
      ...
      limit: 50,
      page: 1,
      published: true, // Solo productos publicados (activos)
      stock_status: 'in_stock' // Solo productos con stock disponible
    });
...
async function handleLink() {
  ...
  linking.value = true;
  const productData = selectedProductIds.value.map(id => ({
    producto_id: id,
    atributo_id: 0 // Sin atributo específico por ahora
  }));
