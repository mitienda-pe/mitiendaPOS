    ...
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col">
      ...
      <div class="bg-red-600 text-white px-6 py-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-xl font-bold">Stock Insuficiente en NetSuite</h3>
            <p class="text-sm opacity-90">No hay stock disponible para completar esta venta</p>
          </div>
        </div>
        <button
          @click="close"
      ...
      <div class="flex-1 overflow-y-auto px-6 py-4">
        <div v-if="unavailableItems && unavailableItems.length > 0" class="space-y-4">
          <p class="text-gray-700 mb-4">
            Los siguientes productos no tienen stock suficiente en NetSuite:
          </p>
          ...
          >
            <div class="flex items-start justify-between gap-4">
              ...
              <div class="text-right">
                ...
                <div class="text-2xl font-bold text-gray-900">{{ item.requested_quantity }}</div>
              </div>
              <div class="text-right">
                <div class="text-sm text-gray-600">Disponible</div>
                <div class="text-2xl font-bold text-red-600">{{ item.available_stock }}</div>
              </div>
            </div>

            <!-- Stock status -->
            <div class="mt-3 pt-3 border-t border-red-200">
              <div v-if="item.available_stock === 0" class="flex items-center gap-2 text-red-700">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                </svg>
                <span class="text-sm font-medium">Producto agotado en NetSuite</span>
              </div>
              <div v-else class="flex items-center gap-2 text-orange-700">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
                </svg>
                <span class="text-sm font-medium">Stock insuficiente (Faltante: {{ item.requested_quantity - item.available_stock }})</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div class="flex gap-3">
            ...
            <div class="text-sm text-blue-800">
              <p class="font-medium mb-1">Opciones:</p>
              <ul class="list-disc list-inside space-y-1 ml-2">
                <li>Reduce la cantidad de los productos en el carrito</li>
                <li>Elimina los productos sin stock</li>
                <li>Contacta al administrador para sincronizar el inventario</li>
              </ul>
            </div>
          </div>
        </div>
