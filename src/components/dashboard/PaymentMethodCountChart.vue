<template>
  <div class="bg-white rounded-lg shadow p-3 sm:p-6">
    <h2 class="text-lg font-medium text-gray-900 mb-4">Operaciones por método de pago</h2>
...
const option = computed(() => ({
  tooltip: {
    trigger: 'item',
    formatter: (p) =>
      `<b>${p.name}</b><br/>Operaciones: ${Number(p.value).toLocaleString('es-PE')}<br/>${p.percent}% del total`
  },
  legend: {
    bottom: 0,
    type: 'scroll',
